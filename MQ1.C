...
unsigned int zero,pi,jing,pre;
unsigned int sum,times;
unsigned long filter_sum;
...
test()
{
    ...
    zero=0;
    while(1){
        a[0]=TABLE[(result/16)%16];
        a[1]=TABLE[result%16];
        i=(result/4096)%16;
        a[2]=(i!=0)?TABLE[i]:0xff;
        a[3]=TABLE[(result/256)%16];
        a[4]=TABLE[10];
        a[5]=TABLE[10];
        i=10;
        a[6]=TABLE[i];
...
/*        display=ON;*/
        ...
        /*a[4]=TABLE[(result/16)%16];
        a[5]=TABLE[result%16];
        i=(result/4096)%16;
        a[6]=(i!=0)?TABLE[i]:0xff;
        a[7]=TABLE[(result/256)%16];
          */
/*        display=~ON;*/
        sbuf(a[0]);
        sbuf(a[1]);
